...
class TradingAgent:
    ...
    def _decide(self, context, assets, has_active_trades=False):
        ...
        system_prompt = (
            ...
            "SOURCE OF TRUTH: account.positions and account.active_trades reflect LIVE exchange + system state. account.recent_diary is the last 10 raw journal entries which may include positions that have since been closed (manual close, SL/TP fill, weekend auto-close, reconciliation). When they disagree, trust positions and active_trades — never claim a position exists based on diary alone.\n\n"
            "Per-asset performance memory (P3.3): each market_data entry now includes recent_performance — win rate, average win/loss, net PnL, and last-5 outcomes over the last 20 closed trades for that asset. A null value means fewer than 2 records exist (insufficient data — trade normally). When recent_performance shows a persistent losing pattern (>5 trades, win_rate < 30%, net_pnl negative), require EXCEPTIONAL conviction before re-entering. Default to hold unless the current setup is qualitatively different from the prior failures — note in your reasoning how this setup differs.\n\n"
            ...
            "REGIME-CONDITIONAL PLAYBOOK (P4.2): each market_data entry includes a regime block with daily-candle structure. Read it and apply the matching playbook:\n\n"
            "  regime = 'trending-up':\n"
            "    - Bias LONG on pullbacks to EMA20 or key support.\n"
            "    - Avoid counter-trend shorts unless extreme overbought + clear reversal candle.\n"
            "    - TP toward next resistance or trend-extension target.\n\n"
            "  regime = 'trending-down':\n"
            "    - Bias SHORT on bounces to EMA20 or key resistance.\n"
            "    - Avoid counter-trend longs unless extreme oversold + clear reversal candle.\n"
            "    - TP toward next support or trend-extension target.\n\n"
            "  regime = 'ranging':\n"
